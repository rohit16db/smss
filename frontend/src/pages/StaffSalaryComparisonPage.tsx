...
export const StaffSalaryComparisonPage: React.FC = () => {
  ...
  const { data: salaryComparison = [], isLoading, isError, error, refetch } = useQuery({
    queryKey: ['Staff-salary-comparison', filters],
    queryFn: () =>
      reportApi.getStaffSalaryComparison({
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `Staff-salary-comparison-${filters.startDate}-to-${filters.endDate}.csv`;
    a.click();
  };
    ...
    return (
      <div className="report-page">
        <div className="error-container" style={{
          ...
          <p>{error instanceof Error ? error.message : 'Failed to load Staff salary comparison data'}</p>
          <button
            onClick={() => refetch()}
  ...
  return (
    <div className="report-page">
      <div className="report-header">
        <div>
          <h1>Staff Salary Comparison</h1>
          <p>Analyze and compare Staff salaries across periods</p>
        </div>
          ...
          <table className="report-table">
            ...
            <tbody>
              {salaryComparison.map((salary) => (
                <tr key={salary.StaffId}>
                  <td className="Staff-name">{salary.StaffName}</td>
      ...
      {salaryComparison.length > 0 && (
        <div className="report-footer">
          <p>Showing {salaryComparison.length} Staff records</p>
