/**
 * Integration Test: Teacher Management Workflow
 * Tests: Create teacher → Update teacher → Toggle status → Delete teacher
...
describe('Teacher Management Integration Tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should display teacher list', async () => {
  ...
  it('should handle teacher search and filtering', async () => {
    ...
    await waitFor(() => {
      expect(api.teacherApi.getAll).toHaveBeenCalledWith(
  ...
  it('should toggle teacher active status', async () => {
    ...
    vi.mocked(api.teacherApi.getAll).mockResolvedValue(mockPaginatedTeachers);
    vi.mocked(api.teacherApi.deactivate).mockResolvedValue(undefined);
    ...
    await waitFor(() => {
      expect(api.teacherApi.deactivate).toHaveBeenCalledWith('1');
    });
  });

  it('should handle teacher deletion', async () => {
    const user = userEvent.setup();
    
    vi.mocked(api.teacherApi.getAll).mockResolvedValue(mockPaginatedTeachers);
    vi.mocked(api.teacherApi.delete).mockResolvedValue(undefined);
    ...
    await waitFor(() => {
      expect(api.teacherApi.delete).toHaveBeenCalled();
    });
  });

  it('should display loading skeleton while fetching data', async () => {
    vi.mocked(api.teacherApi.getAll).mockImplementation(
      () => new Promise((resolve) => setTimeout(() => resolve(mockPaginatedTeachers), 100))
    );
  ...
  it('should handle API errors gracefully', async () => {
    vi.mocked(api.teacherApi.getAll).mockRejectedValue(new Error('Network error'));
